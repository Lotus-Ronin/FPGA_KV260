...
{
	uint8 i;							//用于循环的索引变量
	uint8 rxBuf;					//用于存储从USART1接收到的单个字节数据

	static uint8 startCodeSum = 0;		//用于计数连续接收到的起始码的数量（0x55）
	static bool fFrameStart = FALSE;	//标志一个数据帧的开始
	static uint8 messageLength = 0;		//记录当前接收到的数据帧的长度
	static uint8 messageLengthSum = 2;//根据接收到的数据帧的第二个字节(长度字节)来设置期望的信息长度
	

    if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)			//检查UART1中断标志位非REST（REST）
    {
        rxBuf = USART_ReceiveData(USART1);//(USART1->DR);	//读取接收到的数据（rxBuf）
		if(!fFrameStart)
		{
			if(rxBuf == 0x55)
			{
